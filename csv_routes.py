        ...
        for index, row in df.iterrows():
            try:
                ...
                if transaction_type not in ['income', 'expense']:
                    errors.append(f"Row {index+2}: Invalid type '{row['Type']}'. Must be 'Income' or 'Expense'.")
                    error_count += 1
                    continue
                ...
                if not category:
                    errors.append(f"Row {index+2}: Category cannot be empty.")
                    error_count += 1
                    continue
                
                description = str(row['Description']).strip()
                if not description:
                    errors.append(f"Row {index+2}: Description cannot be empty.")
                    error_count += 1
                    continue
